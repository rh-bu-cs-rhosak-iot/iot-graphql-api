...
const methodMapping: { [method: string]: string } = {
  in: 'whereIn',
  between: 'whereBetween',
  default: 'where'
};

const notMethodMap: { [method: string]: string } = {
  where: 'whereNot',
...
function where(
  ...
  for (const entry of Object.entries(filter)) {
    ...
    if (exprEntry[1] === null) {
      builder =
        builder[builderMethod('whereNull', or, exprEntry[0] === 'ne')](col);
    } else if (Object.keys(methodMapping).includes(exprEntry[0])) {
      builder = builder[builderMethod(exprEntry[0], or, not)](
        col,
        exprEntry[1]
      );
    } else if (exprEntry[0] === 'contains') {
      builder = builder[builderMethod('where', or, not)](
        col,
        mapOperator(exprEntry[0]),
        `%${exprEntry[1]}%`
      );
